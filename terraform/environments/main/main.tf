...
resource "aws_vpc" "main" {
  cidr_block = "10.${index(["dev", "qa", "staging", "prod"], local.main_env) + 1}.${index(["main", "qa", "staging"], local.sub_env) + 1}.0/24"
...
# Route53 Records
# Only keep the wildcard record for all subdomains
...
resource "aws_cloudfront_distribution" "main" {
  ...
  # Temporarily comment out aliases until SSL certificate is validated
