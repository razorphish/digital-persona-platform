...
terraform {
  ...
  backend "s3" {
    ...
    # Format: main/{sub_environment}/terraform.tfstate
  }
}
...
variable "sub_environment" {
  description = "Sub-environment name (e.g., main01, main02)"
  type        = string
  default     = "main01"
}
...
module "rds_proxy" {
  ...
  # RDS Proxy settings optimized for production (main branch)
  idle_client_timeout          = 1800 # 30 minutes
  max_connections_percent      = 80   # Higher for production
  max_idle_connections_percent = 40   # Lower idle for efficiency
  connection_borrow_timeout    = 120  # 2 minutes
  require_tls                  = true # Enable TLS for production
...
module "aws_batch_ml" {
  ...
  # Batch compute configuration (production settings for Main)
  min_vcpus           = 0
  max_vcpus           = 20
  desired_vcpus       = 0
