...
const DOCUMENT_TYPES: DocumentType[] = [
  ...
  },
  {
    type: 'selfie_with_id',
    name: 'Selfie with ID',
    description: 'Photo of yourself holding your government ID next to your face',
    required: true,
...
export default function DocumentUploadStep({
  ...
  const handleFileUpload = useCallback(async (file: File, documentType: string) => {
    ...
    if (file.size > maxSize) {
      setUploadErrors(prev => ({ ...prev, [documentType]: 'File size must be less than 10MB' }));
      return;
    }

    if (!allowedTypes.includes(file.type)) {
      setUploadErrors(prev => ({ ...prev, [documentType]: 'File must be JPEG, PNG, or PDF' }));
      return;
    }

    setUploadingFiles(prev => new Set([...prev, documentType]));
      ...
      <div className="max-w-4xl mx-auto">
        ...
        <div className="mb-8">
          ...
          <p className="text-gray-600">
            Upload clear photos of your documents for identity verification. All uploads are encrypted and secure.
          </p>
        </div>
        ...
        <div className="mb-6 bg-amber-50 border border-amber-200 rounded-lg p-4">
          <div className="flex">
            ...
            <div className="ml-3">
              <h3 className="text-sm font-medium text-amber-800">Photo Guidelines</h3>
              ...
              <div key={docType.type} className="border border-gray-200 rounded-lg p-4">
                <div className="flex items-start justify-between mb-3">
                  <div>
                    ...
                    {docType.example && (
                      <p className="text-xs text-gray-500 mt-1">{docType.example}</p>
                    )}
                  </div>
                ...
                <div
                  className={`
                    ...
                    ${isDraggedOver ? 'border-indigo-400 bg-indigo-50' : 
                      uploadedDoc?.uploaded ? 'border-green-300 bg-green-50' :
                ...
                >
                  ...
                  {isUploading ? (
                    ...
                    <div className="flex flex-col items-center">
                      ...
                      <p className="text-xs text-green-600">{uploadedDoc.file?.name}</p>
                      <button
        ...
        <div className="mb-6 bg-blue-50 border border-blue-200 rounded-lg p-4">
          <div className="flex">
            ...
            <div className="ml-3">
              <h3 className="text-sm font-medium text-blue-800">Facial Verification</h3>
              <p className="mt-1 text-sm text-blue-700">
                After uploading your documents, we'll use AI to verify that your selfie matches your government ID. 
                This helps prevent identity fraud and keeps the platform secure.
              </p>
            </div>
          </div>
        </div>
        ...
        {!canContinue && (
          <div className="mt-4 text-center">
            <p className="text-sm text-gray-500">
              Please upload all required documents to continue ({DOCUMENT_TYPES.filter(dt => dt.required).length - 
              DOCUMENT_TYPES.filter(dt => dt.required).filter(dt => getUploadedDocument(dt.type)?.uploaded).length} remaining)
            </p>
          </div>
        )}
      </div>
