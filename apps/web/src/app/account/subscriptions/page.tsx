...
function SubscriptionManagementContent() {
  ...
  // tRPC queries
  const { data: subscriptions, isLoading, refetch } = trpc.creatorMonetization.getUserSubscriptions.useQuery();
  const { data: paymentMethods } = trpc.creatorMonetization.getPaymentMethods.useQuery();

  // Mutations
  const cancelSubscription = trpc.creatorMonetization.cancelSubscription.useMutation();
  const reactivateSubscription = trpc.creatorMonetization.reactivateSubscription.useMutation();
  ...
  const handleReactivateSubscription = async (subscriptionId: string) => {
    try {
      const result = await reactivateSubscription.mutateAsync({ subscriptionId });
      if (result.success) {
        refetch();
      }
    } catch (error) {
        ...
        <div className="mb-8">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="mt-2 text-gray-600">Manage your persona subscriptions and payment methods</p>
            </div>
            <button
          ...
          {!subscriptions || subscriptions.length === 0 ? (
            <div className="bg-white rounded-lg shadow-sm border p-8 text-center">
              ...
              <p className="text-gray-600 mb-6">
                You don't have any active persona subscriptions yet. Start exploring and connecting with digital personas!
              </p>
              <button
                onClick={() => router.push('/personas')}
                ...
                <div key={subscription.id} className="bg-white rounded-lg shadow-sm border p-6">
                  <div className="flex items-center justify-between">
                    <div className="flex items-center space-x-4">
                      ...
                      <div className="flex-1">
                        <div className="flex items-center space-x-3">
                          <h3 className="text-lg font-medium text-gray-900">{subscription.personaName}</h3>
                          {getStatusBadge(subscription.status, subscription.cancelAtPeriodEnd)}
                        </div>
                        <div className="mt-1 text-sm text-gray-600 space-y-1">
                          <p>
                            <span className="font-medium">${subscription.amount}/month</span> • {subscription.subscriptionTier} tier
                          </p>
                          <p>
                          ...
                          {subscription.cancelAtPeriodEnd && (
                            <p className="text-yellow-600">
                              ⚠️ This subscription will end on {formatDate(subscription.currentPeriodEnd)}
                            </p>
                          )}
                        </div>
                      </div>
                    ...
                    <div className="flex-shrink-0 space-x-3">
                      <button
                        onClick={() => router.push(`/chat?personaId=${subscription.personaId}`)}
                        className="bg-indigo-600 text-white px-4 py-2 rounded-lg hover:bg-indigo-700 transition-colors"
                      >
                        Chat Now
                      </button>
        ...
        <div>
          ...
          <div className="bg-white rounded-lg shadow-sm border p-6">
            <p className="text-gray-600 text-center py-4">
              Billing history will appear here once you have active subscriptions.
            </p>
          </div>
        </div>
