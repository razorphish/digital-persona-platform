...
export interface VerificationDocument {
  type: 'government_id_front' | 'government_id_back' | 'selfie_with_id' | 'utility_bill' | 'bank_statement' | 'tax_document';
  file: Buffer;
  fileName: string;
  mimeType: string;
}
...
export class CreatorVerificationService {
  ...
  constructor() {
    ...
    this.encryptionKey = process.env.VERIFICATION_ENCRYPTION_KEY || 'default-key-change-in-production';
  }

  /**
   * Start the verification process for a creator
   */
  async startVerification(userId: string, ipAddress: string, userAgent: string): Promise<VerificationResult> {
    try {
  ...
  async submitIdentityVerification(
    ...
    try {
      ...
      const encryptedIdNumber = this.encryptSensitiveData(identityData.governmentIdNumber);
  ...
  async submitBankingVerification(
    ...
    try {
      ...
      const encryptedRoutingNumber = this.encryptSensitiveData(bankingData.routingNumber);
      const encryptedAccountNumber = this.encryptSensitiveData(bankingData.accountNumber);
  ...
  async uploadVerificationDocument(
    ...
    try {
      ...
      const bucket = process.env.VERIFICATION_DOCUMENTS_BUCKET || 'hibiji-verification-docs';
      ...
      if (document.type === 'government_id_front' || document.type === 'government_id_back') {
  ...
  async performFacialVerification(
    ...
    try {
      ...
      const confidenceScore = faceMatches.length > 0 ? faceMatches[0].Similarity || 0 : 0;
   ...
   */
  async submitForReview(userId: string, verificationId: string): Promise<VerificationResult> {
    try {
      ...
      const validationErrors = this.validateVerificationCompleteness(verification);
      ...
      return {
        ...
        verificationId,
        status: 'in_review',
        nextSteps: ['Wait for admin review', 'Review typically takes 1-3 business days']
      };
  ...
  async getVerificationStatus(userId: string): Promise<{
    ...
    try {
      ...
      return {
        ...
        nextSteps: this.getNextSteps(verificationRecord.status),
  ...
  private async updateVerificationDocumentIds(verificationId: string, documentId: string) {
    const verification = await this.getVerificationById(verificationId);
  ...
  private async processIdDocumentWithAI(documentId: string, bucket: string, s3Key: string) {
    try {
      ...
      const extractedText = textResponse.TextDetections?.map(t => t.DetectedText).join(' ') || '';
  ...
  private validateVerificationCompleteness(verification: any): string[] {
    ...
    if (!verification.governmentIdType) errors.push('Government ID type is required');
    ...
    if (!verification.faceVerificationScore) errors.push('Facial verification is required');

    const documentIds = verification.verificationDocumentIds || [];
    if (documentIds.length < 3) errors.push('Minimum 3 documents required (ID front, ID back, selfie)');
