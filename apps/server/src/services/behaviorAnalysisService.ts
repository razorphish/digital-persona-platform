...
interface BehaviorPattern {
  userId: string;
  patternType: 'spam' | 'harassment' | 'escalation' | 'inappropriate_content' | 'normal';
  confidence: number;
  indicators: string[];
  riskLevel: 'low' | 'medium' | 'high' | 'critical';
  recommendedAction: 'none' | 'warning' | 'rate_limit' | 'temporary_restriction' | 'account_suspension';
}
...
interface ThreatIndicator {
  type: 'language_threat' | 'behavior_escalation' | 'spam_pattern' | 'harassment_pattern';
...
export class BehaviorAnalysisService {
   ...
   */
  async analyzeUserBehavior(userId: string, timeframeHours: number = 24): Promise<BehaviorPattern> {
    try {
      ...
      const threatIndicators = await this.detectThreatIndicators(userId, timeframe);
      ...
      if (pattern.riskLevel !== 'low') {
        await this.recordBehaviorAnalysis(userId, pattern, metrics, threatIndicators);
      }
   ...
   */
  private async gatherBehaviorMetrics(userId: string, timeframe: Date): Promise<BehaviorMetrics> {
    const [
      messageStats,
      interactionStats,
      violationStats,
   ...
   */
  private async getMessageStats(userId: string, timeframe: Date): Promise<{
    frequency: number;
    avgLength: number;
    sentiment: number;
  }> {
    try {
      ...
      const avgLength = messageDetails.length > 0 
        ? messageDetails.reduce((sum, msg) => sum + (msg.content?.length || 0), 0) / messageDetails.length
        : 0;

      // Basic sentiment analysis (simplified)
      const sentiment = this.calculateSentimentScore(messageDetails.map(m => m.content || ''));
   ...
   */
  private async getInteractionStats(userId: string, timeframe: Date): Promise<{ count: number }> {
    try {
   ...
   */
  private async getViolationStats(userId: string, timeframe: Date): Promise<{ count: number }> {
    try {
   ...
   */
  private async getReportStats(userId: string, timeframe: Date): Promise<{ count: number; escalations: number }> {
    try {
   ...
   */
  private async detectThreatIndicators(userId: string, timeframe: Date): Promise<ThreatIndicator[]> {
    ...
    const harassmentIndicator = await this.detectHarassmentPattern(userId, timeframe);
    if (harassmentIndicator) indicators.push(harassmentIndicator);

    // Check for behavior escalation
    const escalationIndicator = await this.detectBehaviorEscalation(userId, timeframe);
    if (escalationIndicator) indicators.push(escalationIndicator);

    // Check for threatening language
    const threatIndicator = await this.detectThreateningLanguage(userId, timeframe);
   ...
   */
  private async detectHarassmentPattern(userId: string, timeframe: Date): Promise<ThreatIndicator | null> {
    try {
   ...
   */
  private async detectBehaviorEscalation(userId: string, timeframe: Date): Promise<ThreatIndicator | null> {
    try {
      ...
      const incidents = await this.db
        ...
        .where(
          and(
            eq(safetyIncidents.userId, userId),
            gte(safetyIncidents.createdAt, new Date(Date.now() - 7 * 24 * 60 * 60 * 1000)) // Last 7 days
          )
        )
      ...
      for (let i = 1; i < incidents.length; i++) {
        const current = severityLevels[incidents[i - 1].severity as keyof typeof severityLevels];
        const previous = severityLevels[incidents[i].severity as keyof typeof severityLevels];
   ...
   */
  private async detectThreateningLanguage(userId: string, timeframe: Date): Promise<ThreatIndicator | null> {
    try {
      ...
      if (moderations.length > 0) {
        const severity = moderations.some(m => m.severity === 'critical') ? 'critical' : 'high';
   ...
   */
  private calculateBehaviorPattern(metrics: BehaviorMetrics, indicators: ThreatIndicator[]): BehaviorPattern {
    let riskLevel: 'low' | 'medium' | 'high' | 'critical' = 'low';
    let patternType: 'spam' | 'harassment' | 'escalation' | 'inappropriate_content' | 'normal' = 'normal';
    let confidence = 0.5;
    let recommendedAction: 'none' | 'warning' | 'rate_limit' | 'temporary_restriction' | 'account_suspension' = 'none';
   ...
   */
  private getPrimaryThreatType(indicators: ThreatIndicator[]): 'spam' | 'harassment' | 'escalation' | 'inappropriate_content' {
  ...
  private async recordBehaviorAnalysis(
    ...
    try {
      ...
      if (profile.length > 0) {
        const updates: any = {
          severityScore: Math.min(1, parseFloat(profile[0].severityScore || '0') + pattern.confidence * 0.1),
          updatedAt: new Date(),
        };
  ...
  private checkRapidMessaging(messages: any[]): boolean {
    ...
    const recentMessages = messages.filter(m => new Date(m.createdAt) > oneHourAgo);
  ...
  private calculateSentimentScore(contents: string[]): number {
    ...
    const negativeWords = ['bad', 'hate', 'angry', 'stupid', 'idiot', 'fuck', 'shit'];
      ...
      const [profile, recentIncidents] = await Promise.all([
        ...
        this.db
          ...
          .where(
            and(
              eq(safetyIncidents.userId, userId),
              gte(safetyIncidents.createdAt, new Date(Date.now() - 7 * 24 * 60 * 60 * 1000))
            )
          ),
      ]);
