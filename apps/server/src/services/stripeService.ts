...
export class StripeService {
  ...
  async createConnectAccount(
    ...
    if (!this.stripe) {
      throw new Error("Stripe not configured. Cannot create Connect account.");
    }
  ...
  async createTimeBasedPayment(
    ...
    if (!this.stripe) {
      throw new Error(
        "Stripe not configured. Cannot create time-based payment."
      );
    }
    ...
    try {
      ...
      const totalAmount =
        Math.round((sessionMinutes / 60) * hourlyRate * 100) / 100; // Round to cents
      const platformFee = Math.round(totalAmount * this.platformFeePercentage);
      const creatorAmount = totalAmount - platformFee;
  ...
  async processCreatorPayouts(): Promise<void> {
    try {
      ...
      for (const account of creatorAccounts) {
        await this.processCreatorPayout(
          account.userId,
  ...
  private async handleInvoicePaymentSuccess(invoice: Stripe.Invoice) {
    if (invoice.subscription) {
      await db
        ...
        .where(
          eq(
            subscriptionPayments.stripeSubscriptionId,
  ...
  private async updateCreatorEarnings(paymentIntent: Stripe.PaymentIntent) {
    ...
    console.log(
      `Creator ${creatorId} earned $${creatorAmount} from payment ${paymentIntent.id}`
    );
  }
